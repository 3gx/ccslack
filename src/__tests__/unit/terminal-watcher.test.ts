...
vi.mock('../../session-reader.js', () => ({
  ...
  hasTextOutput: vi.fn((msg) => {
    // Return true if message has text content
    const content = msg.message?.content;
    if (typeof content === 'string') return content.trim().length > 0;
    if (!content) return false;
    return content.some((b: any) => b.type === 'text' && b.text?.trim());
  }),
}));
...
describe('terminal-watcher', () => {
  ...
  describe('live activity state machine', () => {
    it('should create live activity message when assistant message arrives without text', async () => {
      const mockMessage = {
        type: 'assistant',
        uuid: 'thinking-only-123',
        timestamp: '2024-01-01T00:00:00Z',
        sessionId: 'sess-1',
        message: { role: 'assistant', content: [{ type: 'thinking', thinking: 'deep thoughts...' }] },
      };
      ...
      vi.mocked(sessionReader.hasTextOutput).mockReturnValue(false);
      vi.mocked(sessionReader.buildActivityEntriesFromMessage).mockReturnValue([{
        timestamp: Date.now(),
        type: 'thinking',
        thinkingContent: 'deep thoughts...',
        thinkingTruncated: 'deep thoughts...',
      }]);

      startWatching('channel-1', undefined, mockSession, mockClient, 'status-ts');
      await vi.advanceTimersByTimeAsync(2000);

      // Should post live activity message (not the terminal output)
      expect(mockClient.chat.postMessage).toHaveBeenCalledWith(
        expect.objectContaining({
          channel: 'channel-1',
          ...
          blocks: expect.arrayContaining([
            expect.objectContaining({
              ...
              text: expect.objectContaining({
                text: expect.stringContaining('Watching terminal activity'),
              }),
            }),
          ]),
        })
    ...
    it('should update spinner when no new messages but processing', async () => {
      // First poll: message with thinking only
      ...
      vi.mocked(sessionReader.readNewMessages)
        .mockResolvedValueOnce({ messages: [thinkingMessage], newOffset: 2000 })
        .mockResolvedValue({ messages: [], newOffset: 2000 }); // No new messages on subsequent polls
      ...
      vi.mocked(sessionReader.buildActivityEntriesFromMessage).mockReturnValue([{
        ...
        thinkingTruncated: 'thinking...',
      }]);

      // Setup to track the ts of the activity message
      let activityMsgTs = 'activity-msg-ts';
      mockClient.chat.postMessage.mockImplementation(() =>
        Promise.resolve({ ts: activityMsgTs })
      );

      startWatching('channel-1', undefined, mockSession, mockClient, 'status-ts');

      // First poll - creates activity message
      await vi.advanceTimersByTimeAsync(2000);
      expect(mockClient.chat.postMessage).toHaveBeenCalled();

      // Second poll - no new messages but should update spinner
      await vi.advanceTimersByTimeAsync(2000);
      expect(mockClient.chat.update).toHaveBeenCalled();
    });

    it('should finalize activity when text output arrives', async () => {
      // Message with both thinking and text - arrives in single message
      const fullMessage = {
        type: 'assistant',
        uuid: 'full-123',
        timestamp: '2024-01-01T00:00:00Z',
      ...
      vi.mocked(sessionReader.readNewMessages)
        .mockResolvedValueOnce({ messages: [fullMessage], newOffset: 2000 })
        .mockResolvedValue({ messages: [], newOffset: 2000 });

      vi.mocked(sessionReader.extractTextContent).mockReturnValue('Final response');
      vi.mocked(sessionReader.hasTextOutput).mockReturnValue(true);
      vi.mocked(sessionReader.buildActivityEntriesFromMessage).mockReturnValue([{
        timestamp: Date.now(),
        type: 'thinking',
        thinkingContent: 'thinking...',
      ...
      // First poll - creates activity message and immediately finalizes with text output
      await vi.advanceTimersByTimeAsync(2000);

      // Should have called uploadMarkdownAndPngWithResponse for final response
      expect(streaming.uploadMarkdownAndPngWithResponse).toHaveBeenCalledWith(
        mockClient,
        'channel-1',
        'Final response',
        expect.stringContaining('Terminal Output'),
        undefined,
        undefined,
        500,
        false,
    ...
    it('should include View Log button in live activity message', async () => {
      const mockMessage = {
        type: 'assistant',
        uuid: 'activity-123',
        timestamp: '2024-01-01T00:00:00Z',
      ...
      // Should include View Log button
      expect(mockClient.chat.postMessage).toHaveBeenCalledWith(
        expect.objectContaining({
          blocks: expect.arrayContaining([
            expect.objectContaining({
